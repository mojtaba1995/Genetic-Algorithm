...
float findmax(generation a) {
	...
	for (i = 0; i < choromozomenumber; ++i) {
		...
		z[i][1] = i;
		//st functions 
		if (ch[i].getgene(0).getvalue() > 9) {

			z[i][0] = z[i][0] - 100000;
		}
		if (2 * ch[i].getgene(0).getvalue() + ch[i].getgene(1).getvalue() > 22) {
			z[i][0] = z[i][0] - 100000;
		}
		if (ch[i].getgene(0).getvalue() + 4 * ch[i].getgene(1).getvalue() > 32) {
			z[i][0] = z[i][0] - 100000;
		}
		if (ch[i].getgene(0).getvalue() <= 0) {
			z[i][0] = z[i][0] - 100000;
		}
		if (ch[i].getgene(1).getvalue() <= 0) {
			z[i][0] = z[i][0] - 100000;
		}
	}
...
generation mutate(generation a) {
	//generation b;
	int random, random2, random3, random4;
	int i;
	...
	for (i = 0; i < mutationnumber; ++i) {
		random = rand() / (RAND_MAX / choromozomenumber);
		random2 = rand() / (RAND_MAX / genenumber);
		random3 = rand() / (RAND_MAX / 2);
		random4 = rand() / (RAND_MAX / 2);
		//cout << random3 << "\n";
		//cout << random4<<"\n";
		if (random4 == 1) {
			a.getchorome(random).genemutate(random2, random3);
			//a.getchorome(0).genemutate(0, 1);
			a.getchorome(random).getgene(random2).setvalue(7);
			//cout <<"\n"<< a.getchorome(random).getgene(0).getvalue() << "\n";
			//cout << "\n" << a.getchorome(random).getgene(1).getvalue() << "\n";
			a.showall();
...
float func(choromozome input) {
	...
	//sample function
	//for (i = 0; i < genenumber; ++i) {
	//	result =result+ input.getgene(i).getvalue();
	//}
...
generation randompick(generation main) {
	generation parent;
	int random[percent + 1];
	float z[percent + 1];
	choromozome ch[percent + 1];
	int i, j, k, e;
	float array[percent + 1][2];
	int n, c, d, swap2;
	float swap;

	for (j = 0; j < (choromozomenumber / tops); ++j) {

		for (i = 0; i < percent; ++i) {
			random[i] = rand() / (RAND_MAX / choromozomenumber);
			ch[i].setchorome(main.getchorome(random[i]));
		...
		for (k = 0; k < tops; ++k) {
			...
			parent.setchorome(ch[e], (tops*j + k));
			//	main.setchorome(ch[array[1][0]]);
		}
...
generation nextgen(generation parent) {
	generation child;
	// to do get int parent and make change 
	child.setgeneration(parent);
	return child;
}